...
let importObject = {
  ...
  as_hello: {
    ...
    console_log: function (index) {
      ...
      // I think should be len/2, because 2bytes count as one Uint16 word
      const str_words = new Uint16Array(memory.buffer, index, len/2);
      // decode the utf-16 byte array into a JS string
      const log_string = new TextDecoder("utf-16").decode(str_words);
...
(async () => {
  ...
  memory = obj.instance.exports.memory; // call the HelloWorld function
